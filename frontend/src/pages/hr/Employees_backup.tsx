...
import { 
  ...
  Filter, Download, TrendingUp, Award, Clock
} from 'lucide-react';
...
export default function Employees() {
  ...
  // Only fetch employees once on mount; search is in-memory
  ...
  const filtered = useMemo(() => {
    return employees.filter(e => {
      ...
      const matchesDept = deptFilter === 'all' || (deptMap[e.department] || e.department) === deptFilter;
  ...
  if (loading) return <div className='flex items-center justify-center min-h-[300px] gap-2'><RefreshCw className='h-5 w-5 animate-spin'/><span>Loading employees...</span></div>;
